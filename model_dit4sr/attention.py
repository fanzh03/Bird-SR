...
class FeedForwardControl(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        #     print(f"forward hidden_states.shape: {hidden_states.shape}")
        ...
        for i, module in enumerate(self.net):
            hidden_states = module(hidden_states)
            # print(f"enumerate hidden_states.shape: {hidden_states.shape}")
            if i == 1:
                # print(f"chunk hidden_states.shape: {hidden_states.shape}")
                hidden_states, hidden_states_control_org = hidden_states.chunk(2, dim=1)
                B, N, C = hidden_states.shape
                # print(f"if hidden_states.shape: {hidden_states.shape}")
                h = w = int(np.sqrt(N))
