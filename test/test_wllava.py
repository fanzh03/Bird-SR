...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        # ====== 新增：跳过已存在结果的逻辑 ======
        result_base = os.path.join(args.output_dir, "sample00")
        os.makedirs(result_base, exist_ok=True)

        for image_idx, image_name in enumerate(image_names[:]):
            name, ext = os.path.splitext(os.path.basename(image_name))
            result_path = os.path.join(result_base, f"{name}.png")

            # 如果结果图像已经存在，则跳过处理
            if os.path.exists(result_path):
                print(f"[跳过] {name}.png 已存在于 {result_base}")
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_model_name_or_path", type=str,
                        default='preset/models/stable-diffusion-3.5-medium')
