...
def parse_args(input_args=None):
    ...
    parser.add_argument("--lpips_ckpt",  type=str , default='/PATH/DiT4SR-main/preset/IQAWeights/lpips/vgg16-397923af.pth', help="Need lpips_ckpt !!!!")
    parser.add_argument("--lpips_loss_weight",  type=float , default=0.0001)

    parser.add_argument("--musiq_loss",  type=int , default=0)
    parser.add_argument("--musiq_ckpt",  type=str , default='/PATH/DiT4SR-main/preset/IQAWeights/musiq/musiq_koniq_ckpt-e95806b9.pth' , help="Need musiq_ckpt !!!!")
    parser.add_argument("--musiq_loss_weight",  type=float , default=0.00001)

    parser.add_argument("--clipiqa_loss",  type=int , default=1)
    parser.add_argument("--clipiqa_ckpt",  type=str , default='/PATH/DiT4SR-main/preset/IQAWeights/cilpiqa/RN50.pt' , help="Need clipiqa_ckpt !!!!")
    parser.add_argument("--clipiqa_loss_weight",  type=float , default=0.00005)

    parser.add_argument("--liqe_loss",  type=int , default=0)
    parser.add_argument("--liqe_ckpt",  type=str , default='/PATH/DiT4SR-main/preset/IQAWeights/liqe/liqe_koniq.pt' , help="Need liqe_ckpt !!!!")
    parser.add_argument("--liqe_loss_weight",  type=float , default=0.00001)

    parser.add_argument("--maniqa_loss",  type=int , default=0)
    parser.add_argument("--maniqa_ckpt",  type=str , default='/PATH/DiT4SR-main/preset/IQAWeights/maniqa/ckpt_koniq10k.pt' , help="Need maniqa_ckpt !!!!")
    parser.add_argument("--maniqa_loss_weight",  type=float , default=0.00001)

    parser.add_argument("--dino_loss",  type=int , default=0)
    parser.add_argument("--dino_ckpt",  type=str , default='/PATH/DiT4SR-main/preset/models/DINO/dinov2_vitl14_reg4_pretrain.pth' , help="Need dino_kl_ckpt !!!!")
    parser.add_argument("--dino_type",  type=str , default='vit_large', choices=["vit_small", "vit_base", "vit_large"], help="Need dino_type !!!!")
    parser.add_argument("--dino_loss_weight",  type=float , default=0.00001)
    parser.add_argument("--dino_loss_type",  type=str , default='MSE' ,choices=["Cosine", "MSE"], help="Need dino_loss_type Cosine or MSE !!!!")
    

    parser.add_argument("--validation_image_dir", type=str, default="/PATH/Data/SR/RealLR200")
    parser.add_argument("--validation_prompt_dir", type=str, default="/PATH/Data/SR/llavaCaptionRealLR200/txt")
...
def compute_clipiqa_loss(pred_image, target_image, clipiqa_loss, relative_difference=True):
    clip_score_pred = clipiqa_loss(pred_image.float())  # [0,1]
    clip_score_target = clipiqa_loss(target_image.float())
    if relative_difference:
        score = F.relu(-clip_score_target.mean() + 1) - F.relu(-clip_score_pred.mean() + 1)
    else:
        score = F.relu(-clip_score_pred.mean() + 1)
    return score
...
def compute_dino_loss(pred_image, old_model_pred, dino_loss, loss_type="MSE"):
    ...
    def preprocess(img):
        ...
        # 建议使用 adaptive_avg_pool 或者是保持比例的 resize，这里沿用 interpolate
        return F.interpolate(img_norm, size=(392, 392), mode="bilinear", align_corners=False)

    pred_in = preprocess(pred_image)
    # target_in = preprocess(target_image)  # .detach() # Target 不需要梯度
    old_in = preprocess(old_model_pred)
    # 2. 提取特征
    # 确保 dino_model 处于 eval 模式且不计算梯度（如果是纯特征提取器）
    pred_feat = dino_loss.forward_features(pred_in)['x_norm_patchtokens'] # [B, N, C]
    # target_feat = dino_loss.forward_features(target_in)['x_norm_patchtokens']
    old_feat = dino_loss.forward_features(old_in)['x_norm_patchtokens']

    if loss_type == "Cosine":
        # 使用 Cosine Similarity 作为感知损失
        # 目标是 maximize similarity -> minimize (1 - similarity)
    ...
    return loss
    # if loss_type == "KL":
    #     # DINO 特征没有概率分布，需要 softmax # softmax 应该在最后一维（特征维）
    #     pred_p = F.log_softmax(pred_feat, dim=-1)  # [B, N, dim]
    #     old_p = F.softmax(old_feat, dim=-1)

    #     KL = torch.nn.KLDivLoss(reduction="batchmean")
    #     loss = KL(pred_p, old_p)
    # else:  # loss_type == "Contrast"
    #     # 采用 InfoNCE 对比损失
    #     # pred_feat 作为 positive，target_feat 为 positive，old_feat 可作为负样本
    #     temperature = 0.07

    #     # L2 normalize
    #     pred_normed = F.normalize(pred_feat, dim=2)
    #     target_normed = F.normalize(target_feat, dim=2)
    #     old_normed = F.normalize(old_feat, dim=2)

    #     # token-wise similarity，sum over channel dim (dim=2)
    #     # positive similarity
    #     pos_sim = (pred_normed * target_normed).sum(dim=2)   # [B, N]
    #     # negative similarity（把 old_feat 当负样本）
    #     neg_sim = (pred_normed * old_normed).sum(dim=2)

    #     # 对所有 patch 求平均
    #     pos_sim = pos_sim.mean(dim=1) / temperature   # [B]
    #     neg_sim = neg_sim.mean(dim=1) / temperature
...
def load_validation_pairs(image_dir, prompt_dir):
    ...
    if len(common_keys) == 0:
        raise ValueError("未找到可匹配的 image-txt 文件对，请检查文件名是否一致")
...
def log_validation(transformer_target, args, accelerator, weight_dtype, global_step, validation_image_list, validation_prompt_list, iqa_model=None):
    ...
    if accelerator.is_main_process:
        ...
        # 只取前 50 个
        pairs = pairs[:10]
        # for image_idx, (image_name, prompt_name) in enumerate(zip(validation_image_list[:50], validation_prompt_list[:50])):
        for image_idx, (image_name, prompt_name) in enumerate(pairs):
            ...
            for sample_idx in range(args.sample_times):  
                ...
                print(f'================== process {image_idx} imgs... save to: {args.output_dir}/sample{str(sample_idx).zfill(2)}/{name}.png ===================')
        ...
        if len(score) > 0:
            ...
            print(f'================== step {global_step} validate all process {image_idx+1} images... ===================')
            print(f'================== average iqa_score of validate all process {image_idx+1} images: {avg_score} ===================')
        else:
...
def main(args):
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                # unwrap_model(transformer_old).save_pretrained(os.path.join(output_dir, "transformer_old"))

                # 找到并移除可训练模型的权重。
                # 假设只有 transformer 是可训练的，其权重在 weights 列表中。
                # 由于我们已经手动保存了所有模型，我们需要清空 weights 和 models 列表，
                # 以阻止 accelerator 进行默认的 state_dict 序列化。
                while len(weights) > 0:
                    weights.pop()
                while len(models) > 0:
                    models.pop()
        ...
        def load_model_hook(models, input_dir):
            ...
            # 识别 models 列表中的三个实例
            # 警告：这里假设 models 列表中包含了这三个实例，并且我们通过外部引用 (transformer, transformer_target, transformer_old) 找到它们。
            
            # 1. 加载 transformer (主训练模型)
            ...
            # 3. 加载 transformer_old (KL Loss参考模型)
            load_model = SD3Transformer2DModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="transformer")
            ...
            # 将所有已加载的模型从 models 列表中弹出，避免 accelerate 默认加载
            while len(models) > 0:
                models.pop()
    ...
    clipiqa_loss, lpips_loss, dino_loss = None, None,None
    ...
    if args.lpips_loss:
        ...
        lpips_loss = lpips.LPIPS(net="vgg", model_path='/home/liuyidi/.cache/torch/hub/checkpoints/vgg16-397923af.pth').to(accelerator.device)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                if debug_dimensions and step == 0:
                    ...
                    logger.info(f"  batch['conditioning_pixel_values'] shape: {batch['conditioning_pixel_values'].shape}")
                ...
                # logger.info(f"hidden_states.shape: {noisy_model_input.shape}")
                # logger.info(f"encoder_hidden_states.shape: {prompt_embeds.shape}")
                # logger.info(f"controlnet_image.shape: {controlnet_image.shape}")
                # logger.info(f"pooled_projections.shape: {pooled_prompt_embeds.shape}")
                # B, C, H, W = noisy_model_input.shape
                # noisy_model_input = noisy_model_input.view(B, C, H*W).permute(0, 2, 1)  # [B, N, C]
                ...
                # logger.info(f"model_pred.requires_grad:{model_pred.requires_grad}")
                # pred_image = vae.decode(model_pred, return_dict=False)[0]
                ...
                # target_image = vae.decode(target, return_dict=False)[0]
                target_image = vae.decode(target).sample   # [-1,1]
                # 映射回 [0,1] 范围
                target_image = (target_image + 1) / 2
                target_image = target_image.clamp(0, 1)

                old_model_pred = (old_model_pred / vae.config.scaling_factor) + vae.config.shift_factor
                old_model_pred = old_model_pred.to(dtype=weight_dtype)
                old_model_image = vae.decode(old_model_pred).sample   # [-1,1]
                # 映射回 [0,1] 范围
                old_model_image = (old_model_image + 1) / 2
                old_model_image = old_model_image.clamp(0, 1)
                # logger.info(f"  old_model_image shape: {old_model_image.shape}")
                # logger.info(f"  target_image shape: {target_image.shape}")
                save_image(pred_image, f'{args.output_dir}/pred_image.png')
                save_image(target_image, f'{args.output_dir}/target_image.png')
                # logger.info(f"pred_image.requires_grad:{pred_image.requires_grad}")
                ###################         Compute Reward Loss from pixel image       ##########################
                if args.lpips_loss and lpips_loss is not None:
                    loss_lpips = compute_lpips_loss(pred_image, target_image, lpips_loss) * args.lpips_loss_weight
                    loss_rl += loss_lpips
                    loss_dict["loss_lpips"] = loss_lpips.detach().item()
                if args.clipiqa_loss and clipiqa_loss is not None:
                    loss_clipiqa = compute_clipiqa_loss(pred_image, target_image, clipiqa_loss) * args.clipiqa_loss_weight
                    loss_rl += loss_clipiqa
                    loss_dict["loss_clipiqa"] = loss_clipiqa.detach().item()
                if args.dino_loss and dino_loss is not None:
                    loss_dino = compute_dino_loss(pred_image, old_model_image, dino_loss, args.dino_loss_type) * args.dino_loss_weight
                    loss_rl = loss_rl + loss_dino
                    loss_dict["loss_dino"] = loss_dino.detach().item()
            ...
            logs = {"loss_rl": loss_rl.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
