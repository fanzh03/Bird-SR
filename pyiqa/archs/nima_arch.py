...
class NIMA(nn.Module):
    ...
    def __init__(
        ...
        self.base_model = timm.create_model(
            base_model_name, features_only=True, pretrained=False,
        )
        #  checkpoint_path='/gdata2/fanzh/model/nima/model.safetensors',
        if pretrained_model_path is not None:
            # import torch
            print(f"[NIMA] Loading pretrained weights from {pretrained_model_path} ...")
            state_dict = torch.load(pretrained_model_path, map_location='cpu')

            # 删除不匹配的分类层
            state_dict = {k: v for k, v in state_dict.items() if not k.startswith('classif.')}
            
            missing, unexpected = self.base_model.load_state_dict(state_dict, strict=False)
            print(f"[NIMA] Loaded pretrained weights (ignore classifier): missing={missing}, unexpected={unexpected}")
