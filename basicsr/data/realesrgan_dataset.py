...
class RealESRGANDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if 'gt_path' in opt:
            if isinstance(opt['gt_path'], str):
                ...
                self.paths.extend(sorted([str(x) for x in Path(opt['gt_path']).rglob('*.'+'jpg')]))
                self.paths.extend(sorted([str(x) for x in Path(opt['gt_path']).rglob('*.'+'JPG')]))
            else:
                self.paths.extend(sorted([str(x) for x in Path(opt['gt_path'][0]).rglob('*.'+opt['image_type'])]))
                self.paths.extend(sorted([str(x) for x in Path(opt['gt_path'][0]).rglob('*.'+'jpg')]))
                self.paths.extend(sorted([str(x) for x in Path(opt['gt_path'][0]).rglob('*.'+'JPG')]))
                if len(opt['gt_path']) > 1:
                    for i in range(len(opt['gt_path'])-1):
                        self.paths.extend(sorted([str(x) for x in Path(opt['gt_path'][i+1]).rglob('*.'+opt['image_type'])]))
                        self.paths.extend(sorted([str(x) for x in Path(opt['gt_path'][i+1]).rglob('*.'+'jpg')]))
                        self.paths.extend(sorted([str(x) for x in Path(opt['gt_path'][i+1]).rglob('*.'+'JPG')]))
    ...
    def __getitem__(self, index):
        ...
        # Skip cropping if crop_size is 0
        if crop_pad_size > 0:
            # pad
